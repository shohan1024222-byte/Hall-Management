...
import NativeFirebaseError from "../NativeFirebaseError.js";
import RNFBNativeEventEmitter from "../RNFBNativeEventEmitter.js";
import SharedEventEmitter from "../SharedEventEmitter.js";
import { getReactNativeModule } from '../nativeModule';
import { isAndroid, isIOS } from "../../common/index.js";
import { encodeNullValues } from "../nullSerialization.js";
...
function nativeModuleWrapped(namespace, NativeModule, argToPrepend, isTurboModule) {
  ...
  for (let i = 0, len = properties.length; i < len; i++) {
    ...
    if (typeof nativeModuleObj[property] === 'function') {
      native[property] = nativeModuleMethodWrapped(namespace, nativeModuleObj[property], argToPrepend, isTurboModule);
    } else {
...
function initialiseNativeModule(module) {
  ...
  for (let i = 0; i < nativeModuleNames.length; i++) {
    ...
    Object.assign(multiModuleRoot, nativeModuleWrapped(namespace, nativeModule, argToPrepend, isTurboModule));
  }
...
function subscribeToNativeModuleEvent(eventName) {
  if (!NATIVE_MODULE_EVENT_SUBSCRIPTIONS[eventName]) {
    RNFBNativeEventEmitter.addListener(eventName, (...args) => {
      const event = args[0];
